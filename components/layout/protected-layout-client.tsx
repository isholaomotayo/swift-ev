    ...
    <div className="flex min-h-screen flex-col bg-background selection:bg-electric-blue/30">
      ...
      <div className="container mx-auto px-4 py-8 flex flex-1 gap-8">
        ...
        <main className="flex-1 min-w-0">
          ...
          <div className="lg:hidden w-full border-b border-border/50 bg-card rounded-2xl p-2 mb-6 overflow-x-auto scrollbar-none flex gap-1 whitespace-nowrap shadow-sm">
            {navItems.map((item) => {
              const Icon = item.icon;
              const isActive = pathname === item.href;
              return (
                <Link key={item.href} href={item.href} className="inline-block">
                  <div
                    className={cn(
                      "flex items-center px-4 py-2.5 rounded-xl text-sm font-bold uppercase tracking-wider transition-all duration-200",
                      isActive
