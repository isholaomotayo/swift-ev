...
export default function AdminAuctionsPage() {
  ...
  const [allAuctions, setAllAuctions] = useState<any[] | null>(null);
  const [loading, setLoading] = useState(true);

  // Fetch once on mount (no real-time subscription)
  useEffect(() => {
    let mounted = true;
    setLoading(true);
    
    convex.query(api.auctions.listAuctions, {})
      .then((data) => {
        if (mounted) {
          setAllAuctions(data || []);
          setLoading(false);
  ...
  const liveAuctions = useMemo(() => 
    allAuctions?.filter(a => a.status === "live") || null,
    [allAuctions]
  );
  
  const scheduledAuctions = useMemo(() => 
    allAuctions?.filter(a => a.status === "scheduled") || null,
    [allAuctions]
  );
  
  const completedAuctions = useMemo(() => 
    allAuctions?.filter(a => a.status === "ended") || null,
    [allAuctions]
  );
  ...
  const renderAuctionTable = (auctions: typeof allAuctions) => {
      ...
      <Tabs defaultValue="all" className="w-full">
        ...
        <TabsContent value="all" className="mt-6">
          <div className="bg-background border rounded-lg overflow-hidden">
            {renderAuctionTable(allAuctions)}
          </div>
        </TabsContent>
