...
export default function VehicleDetailPage() {
  ...
  // Initial one-time fetch to check if auction is active
  const [initialVehicle, setInitialVehicle] = useState<any | null>(null);
  const [isActive, setIsActive] = useState(false);
  const [initialLoading, setInitialLoading] = useState(true);

  // Fetch once to determine if we need real-time subscription
  useEffect(() => {
    let mounted = true;
    setInitialLoading(true);

    convex.query(api.vehicles.getVehicleById, { vehicleId })
      .then((data) => {
        if (mounted) {
          setInitialVehicle(data);
          // Check if auction is active
          const auctionActive = data?.auctionLot?.status === "active";
  ...
  // Use real-time data if available and active, otherwise use initial fetch
  const vehicle = isActive && realtimeVehicle !== undefined ? realtimeVehicle : initialVehicle;

  // Show loading state while fetching initial data
  if (initialLoading || vehicle === null) {
    return (
