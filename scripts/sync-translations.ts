...
languages.forEach((lang) => {
  ...
  enKeys.forEach((key) => {
    if (langKeysSet.has(key)) {
      ...
      // Add missing key (using English text as fallback for inlang to detect)
      newMessages[key] = enMessages[key];
      added++;
    }
  });
...
console.log("\n⚠️  Note: Added keys use original English text.");
console.log("   Inlang will detect these as missing translations and offer auto-translation.");
